...
**Vocabulary** is defined in the sibling doc [`GLOSSARY.md`](GLOSSARY.md) — the ubiquitous
language (what the terms mean); this doc is the data-flow map (how they're computed).
...
**How to read this.**
- **§0 — diagrams**: the big picture (provider → source → transform → metric). Start here.
- **§1–§3 — reference tables**: grep these when tracing a *specific* number back to its source.
- **§4 — reconciliation**: where the same concept is computed two ways, plus the rules that
  keep them honest (incl. §4.1, the rolling-7d vs ISO-week window policy).
- **§5 — maintenance**: dead context keys to prune.

---

## 0. Lineage diagrams

> **Reading the diagrams:** `x · c · h` (in the EffortDataset node) are the model's three
> inputs — `x` = effort distance, `c` = chronic-load fitness, `h` = effort HR. Nodes prefixed
> `M_` are dashboard metrics; the rest are intermediate derived values. Green = the canonical
> value to converge on; red = a competing path that can disagree (the §4 backlog).
...
The dashboard has four tabs — **Overview · Profile · Training · Readiness/Coach** — and the
sections below follow them. All builders take `conn`, live in `fit/report/sections/`; charts
are in `charts.py:_all_charts`.
...
## 4. Reconciliation — one concept, one source of truth

**The rule.** A given concept is computed in exactly **one** place; every consumer reads that
source. Any deviation must be both *deliberate* and *labelled* (e.g. an ISO-week trend chart
beside a rolling-7d card). `Dn` below = a tracked "same concept, two computations" item — a
place where values could otherwise disagree.

**Still open:**
- **D4 — durability keeps two measured signals + one model bound, by design.** Resilience
  (HR-decoupling onset) and Pace-fade (speed give-back) measure *different* things; `β_d` is
  the optimistic power-law bound. The dashboard leads with the measured signals and shows
  `β_d` as a bound — kept distinct on purpose, so nobody collapses them into one number.
- **D15 — `inverse_vdot` alias** (`fitness.py`) just calls `compute_vdot_from_race`: a
  redundant name, no behavioural divergence. Cosmetic; a rename is a candidate OpenSpec change.

**Resolved** — one line each; git history holds the full rationale and dates.

| # | Concept | Resolution |
|---|---------|-----------|
| D1 | VDOT → marathon time | `_vdot_to_marathon_seconds` table **deleted**; forecast = durability model, paces = formula inverse |
| D2 | current VDOT / aerobic capacity | one `_effective_vdot` (anchor > race ≤180d > garmin−5); Garmin VO2max reference-only |
| D3 | marathon/race prediction | every surface reads the one durability-model forecast; degrades to the anchor, never the table |
| D5 | cardiac-drift onset | five algorithms → one grade-adjusted `compute_cardiac_drift` |
| D6 | ACWR | card/coaching/CLI use `compute_rolling_acwr` on the shared chronic-load primitive; `weekly_agg.acwr` is the ISO-week trend |
| D7 | weekly volume / objectives | rolling-7d via `compute_rolling_week` (streak stays ISO-week) |
| D8 | Z2 / easy-% compliance | rolling-7d on the dashboard; `chart-zones` stays per-ISO-week by design |
| D9 | plan adherence | one `compute_plan_adherence` (the bespoke COUNT ratio is gone) |
| D10 | weight target | one `_weight_target` query (the broken `metric='weight'` lookup deleted) |
| D11 | cycling load weighting | `total_load` weights cycling consistently with monotony/strain |
| D12 | readiness gate | gate + auto-dismiss both route through `detect_training_gap` |
| D13 | deload / build-streak | one `_count_consecutive_build_weeks` helper |
| D14 | next workouts | `_next_workouts(_enriched)` share `_next_workouts_base` |
| D16 | time window (rolling-7d vs ISO-week) | policy in §4.1 below |

**Justified deviations kept** (deliberate and labelled, not inconsistencies): Riegel
race-checkpoints on the Overview trend (`^1.06` ≈ measured β_d 1.07, a labelled diagnostic);
`weekly_agg.acwr` as the ISO-week *trend* beside the rolling *card*; MCP `_ctx_training`'s
4-week coaching trend.
...
**Load filters (standing rule).** Monotony, strain, and ACWR's acute term all read
**running-load only**, rolling-7d, computed config-free so an alert's fire and auto-dismiss
yield the identical value. `total_load` itself stays all-activity (it feeds the ISO-week ACWR
trend); the marathon model's fitness covariate also counts all activities (total aerobic
load). ACWR and the model share one `chronic_load_before` primitive (trailing 28d) and differ
only in that activity filter. `weekly_agg` keeps ISO-week copies for the 8-week sparklines.

---

## 5. Maintenance — dead / legacy context keys
...
**Moved to [`GLOSSARY.md`](GLOSSARY.md)** — the ubiquitous language (core vocabulary, the
Physio Calibration concept reference, and the module → bounded-context map) is the repo's
vocabulary SSOT. This doc remains the quantity → function → source **lineage**.
