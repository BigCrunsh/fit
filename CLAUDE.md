...
- **Maximal-effort schedule is duration-keyed (`effort_schedule` → `effort_h_for_distance` in `predict.py`)** — the forecast's assumed race HR is `offset(t)=β·(log t − log T₀)` keyed on the model's predicted **duration**, not distance (distance-keying smuggles in a fitness assumption — the confound the model removes). **T₀** (threshold-duration) is data-driven: a prior (~55 min) updated by the athlete's *at-or-above-threshold* races, recency- and representativeness-weighted, shrinking to the prior when thin. **β stays the population −6.5 prior — deliberately NOT fit per-athlete:** race HRs are contaminated by sub-maximal short efforts (parkruns run *at* threshold, not all-out), so a fitted slope flattens to ≈−2.5 and distorts the whole curve (validated: the 5K would read +3 vs the correct ~+10). Fitting β is the Decision-4 follow-on, gated on a maximality flag (mark which races were all-out); revisit then. All callers go via `effort_h_for_distance` (two-pass coupling); never re-key on distance. The MaxHR-reserve cap rides along. (Separate **D15** still open: the durability Riegel exponent is hardcoded `1.06` in three places vs the fitted β_d — route through one constant.)
