...
- **Aerobic SSOT = effective VDOT (`_effective_vdot`), shown as an integer** — the Physiology card's 4th tile, the Aerobic fitness dimension, the Daniels pace zones, AND the "Needs Your Attention" VDOT-disagreement item all read `_effective_vdot` (confirmed VDOT > recent race ≤180d > Garmin−overestimate). Garmin VO2max reads ~5–10 high and is **reference/diagnostic only** (the Anchor-vs-Garmin chart, the small "Garmin VO2max N" sub-line) — never a headline number. Don't reintroduce raw Garmin VO2max or a raw latest-qualifying-anchor VDOT as a displayed value.
