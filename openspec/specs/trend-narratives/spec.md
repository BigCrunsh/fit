...
- **WHEN** Run Timeline renders with runs in multiple zones
- **THEN** a color legend shows Z1-Z2=blue, Z3=amber, Z4-Z5=orange

### Requirement: Run Timeline gap markers
When consecutive runs in the timeline are more than 14 days apart, a gap marker SHALL appear between them. Format: "-- Nd gap --" in muted text between the run bars. This visually communicates training breaks without distorting the timeline scale.

#### Scenario: 100-day gap shown
- **WHEN** two consecutive runs are 100 days apart
- **THEN** a muted gap marker "-- 100d gap --" appears between the run bars in the timeline

#### Scenario: Short gap not marked
- **WHEN** two consecutive runs are 5 days apart
- **THEN** no gap marker is shown (below 14-day threshold)

### Requirement: WoW as narrative sentence
Week-over-week comparison SHALL be rendered as a narrative sentence generated by `generate_wow_sentence()`, not as a raw data dump of numbers. The sentence synthesizes volume change, zone compliance change, and notable events into natural language.

#### Scenario: Narrative sentence generated
- **WHEN** volume dropped 58% (8km from 19km) and zone compliance went from 0% to 100%
- **THEN** WoW renders: "Volume down 58% (8km from 19km) but zone compliance flipped 0% to 100% — first truly easy week."

#### Scenario: Stable week narrative
- **WHEN** volume changed <5% and zone compliance stable
- **THEN** WoW renders: "Consistent week — volume steady at 25km, zone compliance holding at 80%."

### Requirement: Body tab opening narrative
The Body tab SHALL open with a one-line narrative generated by `generate_body_summary()` that connects recovery signals: readiness trend, RHR stability, HRV direction, and weight vs target. This provides immediate context before the user examines individual charts.

#### Scenario: Mixed recovery signals
- **WHEN** readiness improved 6 to 67, RHR stable, HRV low, weight 3.6kg above target
- **THEN** Body tab opens with: "Recovery improving: readiness 6 to 67. RHR stable. HRV low. Weight: 3.6kg above target."

#### Scenario: All signals positive
- **WHEN** readiness >70, RHR trending down, HRV trending up, weight on target
- **THEN** Body tab opens with: "Recovery strong: readiness high, RHR and HRV trending well, weight on track."
