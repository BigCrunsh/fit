...
Define the per-run detail analysis surfaced on the Training tab as expandable "Last 7 Days" cards. Each card replaces the legacy flat Run Timeline with: header summary (date, type, distance, duration, pace, HR, effort badge, training load), plan comparison vs matched planned workout, per-km split analysis, and 4-week rolling adaptation signals (pace trend, efficiency trend, sRPE context).
