...
class TestHeadlineReadiness:
    ...
    def test_moderate_readiness(self):
        h = generate_headline(readiness=60, acwr=None, phase=None)
        assert "60" in h
        assert "Moderate" in h or "readiness" in h.lower()

    # Unhappy
    def test_low_readiness_recovery(self):
        h = generate_headline(readiness=40, acwr=None, phase=None)
        assert "Recovery" in h or "recovery" in h.lower()

    def test_exactly_75(self):
        h = generate_headline(readiness=75, acwr=None, phase=None)
        assert "Ready" in h

    def test_exactly_50(self):
        h = generate_headline(readiness=50, acwr=None, phase=None)
        assert "50" in h
        # Should NOT be recovery (>=50 is moderate)
        assert "Recovery day" not in h

    def test_exactly_49(self):
        h = generate_headline(readiness=49, acwr=None, phase=None)
        assert "Recovery" in h or "recovery" in h.lower()

    def test_zero_readiness(self):
        h = generate_headline(readiness=0, acwr=None, phase=None)
        assert "Recovery" in h or "recovery" in h.lower()

    def test_none_readiness(self):
        h = generate_headline(readiness=None, acwr=None, phase=None)
        assert "Sync" in h

    def test_exactly_74(self):
        """74 is moderate, not high."""
        h = generate_headline(readiness=74, acwr=None, phase=None)
        assert "Moderate" in h or "74" in h
        assert "Ready for training" not in h

    def test_readiness_100(self):
        h = generate_headline(readiness=100, acwr=None, phase=None)
        assert "Ready" in h
...
class TestHeadlineACWR:
    ...
    def test_acwr_safe(self):
        h = generate_headline(readiness=80, acwr=1.0, phase=None)
    ...
    def test_acwr_0_7_no_warning(self):
        ...
        h = generate_headline(readiness=80, acwr=0.7, phase=None)
        assert "detraining" not in h.lower()
        assert "spike" not in h.lower()

    # Unhappy
    def test_acwr_spike(self):
        h = generate_headline(readiness=80, acwr=1.7, phase=None)
        assert "spike" in h.lower() or "reduce" in h.lower()

    def test_acwr_approaching(self):
        h = generate_headline(readiness=80, acwr=1.4, phase=None)
        assert "1.4" in h or "easy" in h.lower() or "approaching" in h.lower()

    def test_acwr_detraining(self):
        h = generate_headline(readiness=80, acwr=0.5, phase=None)
    ...
    def test_acwr_exactly_1_3(self):
        ...
        h = generate_headline(readiness=80, acwr=1.3, phase=None)
        assert "spike" not in h.lower()
    ...
    def test_acwr_exactly_1_5(self):
        ...
        h = generate_headline(readiness=80, acwr=1.5, phase=None)
    ...
    def test_acwr_exactly_0_6(self):
        ...
        h = generate_headline(readiness=80, acwr=0.6, phase=None)
        assert "detraining" not in h.lower()

    def test_acwr_exactly_0_59(self):
        h = generate_headline(readiness=80, acwr=0.59, phase=None)
    ...
    def test_acwr_zero(self):
        ...
        h = generate_headline(readiness=80, acwr=0, phase=None)
        assert "detraining" in h.lower()

    def test_acwr_very_high(self):
        h = generate_headline(readiness=80, acwr=3.0, phase=None)
        assert "spike" in h.lower() or "reduce" in h.lower()
...
class TestHeadlinePhase:
    ...
    def test_phase1_no_quality(self):
        ...
        h = generate_headline(readiness=80, acwr=1.0, phase=phase)
    ...
    def test_phase_low_readiness_no_quality(self):
        phase = {"name": "Volume", "targets": json.dumps({"quality_sessions_per_week": [1, 2]})}
        h = generate_headline(readiness=55, acwr=1.0, phase=phase)
    ...
    def test_phase_very_low_readiness_no_phase_suggestion(self):
        ...
        phase = {"name": "Volume", "targets": json.dumps({"quality_sessions_per_week": [1, 2]})}
        h = generate_headline(readiness=40, acwr=1.0, phase=phase)
    ...
    def test_phase_exactly_readiness_50(self):
        ...
        phase = {"name": "Volume", "targets": json.dumps({"quality_sessions_per_week": [1, 2]})}
        h = generate_headline(readiness=50, acwr=1.0, phase=phase)
        assert "Volume" in h
...
class TestHeadlineAllNone:
    ...
    def test_low_readiness_high_acwr(self):
        h = generate_headline(readiness=35, acwr=1.7, phase=None)
        assert "Recovery" in h or "recovery" in h.lower()
        assert "spike" in h.lower() or "reduce" in h.lower()
